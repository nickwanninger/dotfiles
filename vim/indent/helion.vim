...
" URL:			https://github.com/vim-helion/vim-helion
...
if !exists('g:helion_indent_access_modifier_style')
  " Possible values: "normal", "indent", "outdent"
  let g:helion_indent_access_modifier_style = 'normal'
endif

if !exists('g:helion_indent_assignment_style')
  " Possible values: "variable", "hanging"
  let g:helion_indent_assignment_style = 'hanging'
endif

if !exists('g:helion_indent_block_style')
  " Possible values: "expression", "do"
  let g:helion_indent_block_style = 'expression'
endif

setlocal nosmartindent

" Now, set up our indentation expression and keys that trigger it.
setlocal indentexpr=GethelionIndent(v:lnum)
setlocal indentkeys=0{,0},0),0],!^F,o,O,e,:,.
setlocal indentkeys+==end,=else,=elif,=when,=ensure,=rescue,==begin,==end
setlocal indentkeys+==private,=protected,=public

" Only define the function once.
if exists("*GethelionIndent")
  finish
endif
...
let s:skip_expr =
      \ 'index(map('.string(s:syng_strcom).',"hlID(''helion''.v:val)"), synID(line("."),col("."),1)) >= 0'

" Regex used for words that, at the start of a line, add a level of indent.
let s:helion_indent_keywords =
      \ '^\s*\zs\<\%(module\|class\|if\|for' .
      \   '\|while\|until\|else\|elif\|case\|when\|unless\|begin\|ensure\|rescue' .
      \   '\|\%(\K\k*[!?]\?\s\+\)\=def\):\@!\>' .
      \ '\|\%([=,*/%+-]\|<<\|>>\|:\s\)\s*\zs' .
      \    '\<\%(if\|for\|while\|until\|case\|unless\|begin\):\@!\>'

" Regex used for words that, at the start of a line, remove a level of indent.
let s:helion_deindent_keywords =
      \ '^\s*\zs\<\%(ensure\|else\|rescue\|elif\|when\|end\):\@!\>'
...
let s:end_middle_regex = '\<\%(ensure\|else\|\%(\%(^\|;\)\s*\)\@<=\<rescue:\@!\>\|when\|elif\):\@!\>'
...
" 2. GethelionIndent Function {{{1
" =========================

function! GethelionIndent(...) abort
  " 2.1. Setup {{{2
  " ----------
...
function! s:AccessModifier(cline_info) abort
  ...
  " class declaration.
  if g:helion_indent_access_modifier_style == 'indent'
  ...
  elseif g:helion_indent_access_modifier_style == 'outdent'
    if s:Match(info.clnum, s:access_modifier_regex)
      let class_lnum = s:FindContainingClass()
      if class_lnum > 0
  ...
  if col > 0 && !s:IsInStringOrComment(info.clnum, col)
    ...
    if searchpair(escape(bracket_pair[0], '\['), '', bracket_pair[1], 'bW', s:skip_expr) > 0
      if closing_bracket == ')' && col('.') != col('$') - 1
        let ind = virtcol('.') - 1
      elseif g:helion_indent_block_style == 'do'
        let ind = indent(line('.'))
      else " g:helion_indent_block_style == 'expression'
...
function! s:DeindentingKeyword(cline_info) abort
  ...
  " TODO: this is messy
  if s:Match(info.clnum, s:helion_deindent_keywords)
    ...
    if searchpair(s:end_start_regex, s:end_middle_regex, s:end_end_regex, 'bW',
      ...
      if s:IsAssignment(line, col('.')) &&
        ...
        if g:helion_indent_assignment_style == 'hanging'
          " hanging indent
          let ind = virtcol('.') - 1
        else
          " align with variable
...
function! s:ContinuedLine(pline_info) abort
  ...
  let col = s:Match(info.plnum, s:helion_indent_keywords)
...
function! s:AfterBlockOpening(pline_info) abort
  ...
  if s:Match(info.plnum, s:block_regex)
    if g:helion_indent_block_style == 'do'
...
function! s:AfterUnbalancedBracket(pline_info) abort
  ...
  if info.pline =~ '[[({]' || info.pline =~ '[])}]\s*\%(#.*\)\=$'
    ...
    elseif closing.pos != -1
      ...
      if s:Match(line('.'), s:helion_indent_keywords)
...
function! s:IndentingKeywordInMSL(msl_info) abort
  ...
  let col = s:Match(info.plnum_msl, s:helion_indent_keywords)
  if col > 0
...
function! s:IsInhelionGroup(groups, lnum, col) abort
  let ids = map(copy(a:groups), 'hlID("helion".v:val)')
  return index(ids, synID(a:lnum, a:col, 1)) >= 0
endfunction

" Check if the character at lnum:col is inside a string, comment, or is ascii.
function! s:IsInStringOrComment(lnum, col) abort
  return s:IsInhelionGroup(s:syng_strcom, a:lnum, a:col)
endfunction

" Check if the character at lnum:col is inside a string.
function! s:IsInString(lnum, col) abort
  return s:IsInhelionGroup(s:syng_string, a:lnum, a:col)
endfunction

" Check if the character at lnum:col is inside a string or documentation.
function! s:IsInStringOrDocumentation(lnum, col) abort
  return s:IsInhelionGroup(s:syng_stringdoc, a:lnum, a:col)
endfunction

" Check if the character at lnum:col is inside a string delimiter
function! s:IsInStringDelimiter(lnum, col) abort
  return s:IsInhelionGroup(
